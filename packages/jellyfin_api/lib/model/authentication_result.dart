...
class AuthenticationResult {
  ...
  @override
  String toString() =>
      'AuthenticationResult[user=$user, accessToken=$accessToken, serverId=$serverId]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "AuthenticationResult[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "AuthenticationResult[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AuthenticationResult> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<AuthenticationResult>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = AuthenticationResult.listFromJson(
          entry.value,
          growable: growable,
        );
      }
