...
| A7: Appearance and persistence | Exercise Fruity/Blockbuster × Soup/Ocean/Grove/Mono × light/dark (16 combinations). Check first-use selection, explicit Apply, switching without losing the destination, settings persistence after restart/sign-out, and fixed-style setup/login. | Blocked: device/account | Partial: first-use focus/Continue verified with fixture; full combinations and persistence checks pending |
